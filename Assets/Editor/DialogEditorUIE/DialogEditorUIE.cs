...
public class DialogEditorUIE : EditorWindow, ISerializationCallbackReceiver {
    ...
    public string GetCurrentCharacterIdle(CharacterData character) {
        IEnumerable<VisualElement> children = scrollView.Children();
        foreach (VisualElement child in children) {
            if (child.GetType() == typeof(DialogEntry)) {
                DialogEntry entry = (DialogEntry)child;
                if ((entry.character == character) && (!string.IsNullOrEmpty(entry.GetIdleClipName()))) {
                    return entry.GetIdleClipName();
                }
            }
        }

        //Debug.Log("Failed to find a previous idle for character " + character.characterName);
        return "";
    }
        ...
        using (FileStream saveFile = new FileStream(saveFilePath, FileMode.Create)) {
            using (BinaryWriter writer = new BinaryWriter(saveFile)) {
                ...
                foreach (VisualElement child in scrollView.Children()) {
                    if (child.GetType() == typeof(DialogEntry)) {
                        //data sanity check, skip if we're serializing
                        if ((saveFilePath == serializationFilePath) || (saveFilePath == PLAYBACK_PREVIEW_PATH)) {
