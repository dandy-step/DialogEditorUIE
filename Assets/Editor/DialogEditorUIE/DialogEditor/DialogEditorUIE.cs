...
[Serializable]
public class DialogEditorUIE : EditorWindow, ISerializationCallbackReceiver {
    ...
    VisualTreeAsset dialogEntryAsset;
    ...
    //AnimationClip clip;
    PreviewRenderUtility render = null;
    ...
    public void OnEnable() {
        ...
        //Button testButton = new Button(AddPreviewWindow) { text = "TEST" };
            ...
            using (BinaryWriter writer = new BinaryWriter(saveFile)) {
                ...
                foreach (VisualElement child in scrollView.Children()) {
                    if (child.GetType() == typeof(DialogEntry)) {
                        ...
                        if ((saveFilePath == serializationFilePath) || (saveFilePath == PLAYBACK_PREVIEW_PATH) || (nodeOwner != null)) {
    ...
    public byte[] GetNodeData() {
        string testFile = "Assets/testFile.node";
        GenerateDialogFile(false, testFile);
        BinaryReader file = new BinaryReader(File.Open(testFile, FileMode.Open));
    ...
    public void GetNodePreview() {
        GUILayout.BeginVertical();
        if (scrollView.childCount == 0) {
            GUILayout.Label("No data!");
        } else {
            for (int i = 0; i < Math.Min(5, scrollView.childCount); i++) {
                GUILayout.Label("TEST: " + scrollView[i].GetType().ToString());
            }
        }
    ...
    private void OnDestroy() {
        if (!string.IsNullOrEmpty(nodeOwner)) {
            Debug.Log("Pass data to node owner");
            ((DialogNode)GetWindow<NodeEdit>().man.GetNodeByID(nodeOwner)).nodeData = GetNodeData();
        }
    }
}
